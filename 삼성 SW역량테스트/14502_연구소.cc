...
1-1. 후보는 0이어야 한다.
1_2. 순서는 오름차순으로 강제
1_3. n*m까지의 배열을 만든 후에 3개의 숫자를 선택 m*nC3 -> 좌표 변환 num = n * x + m; 이 아니라  num = m*x(x는 n 이하) +m;
2. 벽을 세운다.
3. BFS를 실행한다.
4. 벽이 아니면서 방문하지 않은 점의 개수를 센다.
5. 그 값의 최솟값을 반환한다.
// 풀이 후
6. 복사는 생각보다 시간이 오래 걸린다. 이 경우에는 큐를 하나 생성하고 이중 포문을 돌면서 2의 좌표를 다시 전달하는 것이 더 효율적이었다.
7. 0인 것 중에서 방문하지 못하는 것을 세는 것이 훨씬 깔끔한 것 같다.
*/
int n, m;
int board[64][64];
bool visited[64][64];
