...
/*
#17141 : 2의 개수 중에서 m개를 뽑고 BFS를 실행한 후에 최댓값을 센다 -> 50*50 *10C5 -> 600000
다만 연구소 3에서는 2를 제외하고 셌던 것과는 달리 이번에는 2도 조건에 포함시켜서 세줘야 한다.
*/
int virusArr[10];
int board[50][50];
int cnt = 0;
...
int findMaxDist() {
	...
	{
		for (int j = 0; j < n; j++) {
			if ((board[i][j] == 0 || board[i][j] ==2) && dist[i][j] == -1) return MX;
