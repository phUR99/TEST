...
int distS[1005], dist[1005], distE[1005];
int pre[1005];
int N, M;
int MAX = -1;
...
int main(){
    ...
    {
        auto cur = pq.top(); pq.pop();
        if(cur.first > distE[cur.second]) continue;
        for (auto nxt : adj[cur.second])
        {
            if(distE[nxt.second] > cur.first + nxt.first){
                distE[nxt.second] = cur.first + nxt.first;
                pq.push({distE[nxt.second], nxt.second});
            }
        }   
    }
    vector<int> track;
